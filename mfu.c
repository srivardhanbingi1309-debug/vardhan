...
int findMFU(Frame frames[], int n) {
    int maxFreq = frames[0].freq;
    int pos = 0;

    for (int i = 1; i < n; i++) {
        if (frames[i].freq > maxFreq) {
            maxFreq = frames[i].freq;
            pos = i;
        }
        else if (frames[i].freq == maxFreq) {
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (pos != -1) {
            ...
            int replace = findMFU(frames, framesCount);
